...
hashed_flag_parts = [hashed_flag[i:i + part_length] for i in range(0, part_length * 8, part_length)]
...
for i, product in enumerate(products):
    try:
        ...
        logger.info("Assigned ID %s to product %s", hashed_flag_parts[i], product["name"])
    except IndexError:
