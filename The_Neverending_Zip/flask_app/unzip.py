    ...
    try:
        while True:
            ...
            with zipfile.ZipFile(current_zip, 'r') as zipf:
                ...
                if next_zip is None:
                    logger.error(f"No zip file found in layer {layer_count}. Extraction complete but no flag found.")
                    break
            ...
            if current_time - last_report_time > report_interval:
                ...
                logger.info(f"Processed {layer_count} layers in {elapsed:.2f} seconds ({extraction_rate:.2f} layers/sec)")
        ...
        logger.info(f"Total extraction time: {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
        if layer_count > 0 and total_time > 0:
            logger.info(f"Average extraction rate: {layer_count/total_time:.2f} layers/second")
