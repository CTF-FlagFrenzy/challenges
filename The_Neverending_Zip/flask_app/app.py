    ...
    try:
        if os.path.exists(status_file_path):
            with open(status_file_path, "r") as f:
                status = json.load(f)
                if (
                    "estimated_total_time" in status
                    and status["estimated_total_time"] > 0
                ):
...
def index():
    ...
    return render_template(
        "index.html", zip_ready=zip_creation_complete, creating=zip_creation_in_progress
    )
